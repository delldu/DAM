...
def test_input_shape():
    ...
    model, device = anime_face.get_drive_face_generator_model()
...
def export_drive_face_keypoint_onnx_model():
    ...
    # 1. Run torch model
    model, device = anime_face.get_drive_face_keypoint_model()

    B, C, H, W = 1, 3, anime_face.FACE_IMAGE_SIZE, anime_face.FACE_IMAGE_SIZE
...
def export_drive_face_generator_onnx_model():
    ...
    # 1. Run torch model
    model, device = anime_face.get_drive_face_generator_trace_model() # OK trace mode
...
def export_drive_body_onnx_model():
    ...
    # 1. Run torch model
    model, device = anime_face.get_drive_body_generator_model()

    B, C, H, W = 1, 3, anime_face.BODY_IMAGE_SIZE, anime_face.BODY_IMAGE_SIZE
...
def export_drive_mgif_onnx_model():
    ...
    # 1. Run torch model
    model, device = anime_face.get_mgif_model()

    B, C, H, W = 1, 3, anime_face.MGIF_IMAGE_SIZE, anime_face.MGIF_IMAGE_SIZE
    dummy_input1 = torch.randn(B, C, H, W).to(device)
    dummy_input2 = torch.randn(B, C, H, W).to(device)
